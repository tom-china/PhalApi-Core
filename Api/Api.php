<?php
	...
	class Api {
		 ...
		 *
		 * @see Filter::check()
		 * @throws InternalServerError 当验证失败时，请抛出此异常，以返回400
		 */
		protected function filterCheck() {
			$filter = DI()->get( 'filter', \PhalApi\Filter\NoneFilter::class );
			
			if ( isset( $filter ) ) {
				if ( ! ( $filter instanceof IFilter ) ) {
					throw new InternalServerError(
						Translator::get( 'DI()->filter should be instanceof IFilter' ) );
				}
